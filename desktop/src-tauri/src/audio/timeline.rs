...
impl Timeline {
    ...
    pub fn gap(
        ...
        let emitted = if let Some(TimelineEvent::Gap(previous)) = self.events.last_mut() {
            if gaps_are_contiguous(previous, &gap) {
                previous.duration_ms = previous
                    .duration_ms
                    .checked_add(gap.duration_ms)
                    .ok_or(TimelineError::InvalidTiming)?;
                previous.dropped_frames = previous
                    .dropped_frames
...
impl LossSlot {
    ...
    fn record(&self, source_position_frames: u64, dropped_frames: u64, cause: GapCause) {
        ...
        let mut first = self.first_source_position_frames.load(Ordering::Relaxed);
        while source_position_frames < first {
            match self.first_source_position_frames.compare_exchange_weak(
                first,
                source_position_frames,
                Ordering::Relaxed,
                Ordering::Relaxed,
                ...
                Ok(_) => {
                    self.cause.store(encoded_cause, Ordering::Relaxed);
                    break;
                }
                Err(observed) => first = observed,
            }
        }
        if first == EMPTY_SOURCE_POSITION {
        ...
        let _ = self
            ...
            .fetch_update(Ordering::Relaxed, Ordering::Relaxed, |current| {
                Some(current.saturating_add(dropped_frames))
            });
    }

    fn take(&self, generation: u64) -> Option<LossSnapshot> {
...
impl LossAccumulator {
    ...
    fn drain_inner<F>(&self, after_flip: F) -> Option<LossSnapshot>
    where
        F: FnOnce(),
    {
        let generation = self
            ...
            .expect("loss accumulator generation exhausted");
        after_flip();
        let slot = &self.slots[(generation & 1) as usize];
...
fn decode_cause(encoded: u8) -> GapCause {
    match encoded {
        1 => GapCause::CallbackPoolExhausted,
        2 => GapCause::OversizedCallback,
        3 => GapCause::DeviceDiscontinuity,
        4 => GapCause::SinkUnavailable,
        _ => unreachable!("non-empty loss slot must have a cause"),
    }
}
...
mod tests {
    ...
    use crate::audio::frame::{GapCause, TrackConfigurationRevision};
    use crate::audio::session::{SessionId, TrackId};
