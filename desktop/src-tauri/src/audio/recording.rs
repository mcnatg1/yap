...
static MIGRATION_STAGING_COUNTER: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);
...
/// Adopts a validated WAV-only pre-release artifact into the current commit format.
/// A caller must reject legacy transcript pairs before invoking this helper because those
/// transcripts cannot prove the immutable result revision contract.
pub(crate) fn adopt_pre_release_wav(
    directory: &Path,
    legacy_name: &str,
) -> Result<CommittedCapture, String> {
    let mut legacy = open_regular_artifact(directory, legacy_name)?;
    let (_, placeholder) = validate_recoverable_wav(&mut legacy, "pre-release live audio")?;
    if placeholder {
        return Err("pre-release live audio still has a partial WAV header".into());
    }
    let audio_bytes = legacy
        .metadata()
        .map_err(|error| format!("Failed to inspect pre-release live audio: {error}"))?
        .len();
    let audio_sha256 = sha256_open_file(&mut legacy)?;
    let session_id = migration_session_id(&audio_sha256)?;
    let audio_file = format!("live-{session_id}.wav");
    let sidecar_file = format!("live-{session_id}.capture.json");
    let commit_file = format!("live-{session_id}.commit.json");

    ensure_migrated_audio(
        directory,
        legacy_name,
        &legacy,
        &audio_file,
    ...
    let sidecar = CaptureSidecar {
        ...
        sequence_gap_overflow: None,
        sink_degraded: false,
        directory_sync_supported: sync_parent_directory(directory),
        session_metadata: None,
    };
    let sidecar_sha256 = ensure_migrated_json(
        directory,
        &sidecar_file,
        &sidecar,
        "pre-release capture sidecar",
    )?;

    let capture = if open_regular_artifact(directory, &commit_file).is_ok() {
        let manifest = read_manifest(directory, &commit_file)?;
        let capture = validate_committed_capture(directory, manifest)?;
        if !migration_capture_matches(
            &capture,
            &session_id,
            &audio_file,
            &audio_sha256,
            ...
            return Err("pre-release migration destination already has a different commit".into());
        }
        capture
    } else {
        let manifest = CaptureCommitManifest {
            ...
            audio_sha256: audio_sha256.clone(),
            audio_bytes,
            capture_sidecar_file: sidecar_file.clone(),
            capture_sidecar_sha256: sidecar_sha256.clone(),
            committed_at_utc: now_utc()?,
            session_metadata: None,
        };
        manifest.validate()?;
        let commit_sha256 = ensure_migrated_json(
            directory,
            &commit_file,
            &manifest,
            "pre-release capture commit",
        )?;
        let published = read_manifest(directory, &commit_file)?;
        let capture = validate_committed_capture(directory, published)?;
        if !migration_capture_matches(
            &capture,
            &session_id,
            &audio_file,
            ...
            &sidecar_file,
            &sidecar_sha256,
        ) || sha256_regular_artifact(directory, &commit_file)? != commit_sha256
        {
            return Err("pre-release migration commit could not be verified".into());
        }
        capture
    };
    drop(legacy);
    remove_regular_artifact_if_hash(directory, legacy_name, &audio_sha256)?;
    Ok(capture)
}

fn migration_session_id(audio_sha256: &str) -> Result<SessionId, String> {
...
fn migration_capture_matches(
    ...
    manifest.session_id == *session_id
        ...
        && manifest.audio_sha256 == audio_sha256
        && manifest.audio_bytes == audio_bytes
        && manifest.capture_sidecar_file == sidecar_file
        && manifest.capture_sidecar_sha256 == sidecar_sha256
        && manifest.session_metadata.is_none()
}

fn ensure_migrated_audio(
    directory: &Path,
    legacy_name: &str,
    legacy: &File,
    ...
    if let Ok(mut destination) = open_regular_artifact(directory, destination_name) {
        if destination
            .metadata()
            .map_err(|error| format!("Failed to inspect migrated live audio: {error}"))?
            .len()
            != expected_bytes
            || sha256_open_file(&mut destination)? != expected_sha256
        {
            return Err("pre-release migration destination audio does not match the source".into());
        }
        let (_, placeholder) = validate_recoverable_wav(&mut destination, "migrated live audio")?;
        if placeholder {
            return Err("pre-release migration destination audio is still partial".into());
        }
        return Ok(());
    }

    let (staging_path, staging) = create_migration_audio_staging(directory, legacy_name, legacy)?;
    let destination = directory.join(destination_name);
    let published = publish_no_replace(
        &staging_path,
        &destination,
        &staging,
        "publish pre-release live audio",
    );
    if published.is_err() {
        remove_owned_staging(&staging_path, &staging, "publish pre-release live audio");
    }
    drop(staging);
    drop(published?);
    Ok(())
...
fn create_migration_audio_staging(
    directory: &Path,
    legacy_name: &str,
    legacy: &File,
) -> Result<(PathBuf, File), String> {
    for _ in 0..1_024 {
        let staging = migration_staging_path(directory, legacy_name, "audio");
        match fs::hard_link(directory.join(legacy_name), &staging) {
            Ok(()) => {
                let staged = open_regular_path(&staging)?;
                if !same_file_identity(legacy, &staged)? {
                    remove_owned_staging(&staging, &staged, "stage pre-release live audio");
                    return Err(
                        "pre-release migration staging no longer names the verified source".into(),
                    );
                }
                return Ok((staging, staged));
            }
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => continue,
            Err(error) => return Err(format!("Failed to stage pre-release live audio: {error}")),
        }
    }
    Err("Failed to allocate pre-release live audio staging".into())
}

fn ensure_migrated_json<T>(
    directory: &Path,
    destination_name: &str,
    expected: &T,
    label: &str,
...
{
    if let Ok((text, sha256)) = read_and_hash_regular_artifact(directory, destination_name) {
        let found: T = serde_json::from_str(&text)
            .map_err(|error| format!("Failed to parse existing {label}: {error}"))?;
        if &found != expected {
            return Err(format!(
                "existing {label} does not match the migration source"
            ));
        }
        return Ok(sha256);
    }
    ...
    let (staging_path, staging) =
        create_migration_json_staging(directory, destination_name, expected, label)?;
    let destination = directory.join(destination_name);
    let published = publish_no_replace(&staging_path, &destination, &staging, label);
    if published.is_err() {
        remove_owned_staging(&staging_path, &staging, label);
    }
    drop(staging);
    let mut published = published?;
    let sha256 = sha256_open_file(&mut published)?;
    let text = read_open_file(&mut published)
        .map_err(|error| format!("Failed to read {label}: {error}"))?;
    let found: T =
        serde_json::from_str(&text).map_err(|error| format!("Failed to parse {label}: {error}"))?;
    if &found != expected {
        return Err(format!(
            "published {label} does not match the migration source"
        ));
    }
    Ok(sha256)
}

fn create_migration_json_staging<T>(
    directory: &Path,
    destination_name: &str,
    value: &T,
    label: &str,
...
{
    for _ in 0..1_024 {
        let staging = migration_staging_path(directory, destination_name, "json");
        match write_json_file_open(&staging, value, label) {
            Ok(file) => {
                file.sync_all()
                    .map_err(|error| format!("Failed to sync {label}: {error}"))?;
                return Ok((staging, file));
            }
            Err(_error) if staging.exists() => continue,
            Err(error) => return Err(error),
        }
    }
    Err(format!("Failed to allocate {label} staging"))
}

fn migration_staging_path(directory: &Path, destination_name: &str, kind: &str) -> PathBuf {
    let counter = MIGRATION_STAGING_COUNTER.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    directory.join(format!(
        "{destination_name}.migration-{kind}-{:x}-{:x}.part",
        std::process::id(),
        counter
    ))
}
