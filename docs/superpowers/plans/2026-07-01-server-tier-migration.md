...
- **Team / server profile** — thin Tauri client + on-prem **DGX Spark** server tier (GPU model pools, workload router, knowledge compiler, auth).
...
| DGX Spark GPU pool | Multiple concurrent workers | Parallelises across users; frees client CPU |
...
|------------|----------|---------|
| **0014** | `0014-server-tier-compute-topology.md` | Establish the server tier on DGX Spark: thin client shell, workload router, model pools (streaming ASR / Cohere batch / LLM), two deployment profiles (solo local vs team server), on-prem-not-cloud framing, live path tradeoffs |
...
| **0018** | `0018-three-repo-topology.md` | Formal three-repo split: `yap-desktop` (thin client), `yap-server` (DGX brain + IaC), `yap-knowledge` (Git KB source-of-truth); optional future `yap-contracts`; mapping of current `cohere-transcribe-local` workspace |
...
| **0005** llama-server agents | Add amendment note: in team profile, llama-server moves to the LLM pool on the DGX Spark server; solo profile keeps bundled llama-server unchanged; CPU-first rule remains for solo. |
...
|-------|-------|-------------|-----|
| **8** | server | Server tier stand-up: DGX Spark, workload router, streaming ASR pool (Moonshine GPU), Cohere batch pool (concurrent workers); thin-client audio streaming connector | ADR 0014, 0018 |
...
1. **DGX Spark network topology** — Is the server on a flat LAN, VPN-only, or mTLS-gated? Matters for the audio-streaming protocol choice (WSS/QUIC) and offline-fallback trigger. Deferred to ADR 0014 implementation phase.
