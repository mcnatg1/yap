...
- Existing single-microphone settings and legacy `live-<timestamp>[-suffix].wav/.txt` artifacts remain readable.
...
| Dictation and legacy artifacts remain compatible | Tasks 1, 7, and 8 |
...
`list_saved_live_sessions` discovers new sessions from hash-valid `.commit.json` files, validates the highest transcript result revision when present, and reads creation time from committed metadata. It then separately scans readable legacy timestamp-named WAV/TXT pairs. Do not teach `created_at_ms_from_live_stem` to guess the new opaque ID format. Add `list_recoverable_live_sessions` for private partial artifacts. Validate every path remains inside the effective Yap recordings directory before returning it.
...
- [ ] **Step 5: Preserve legacy history and playback**

Extend `SavedLiveSession` and `TranscriptHistoryEntry` with optional `captureCommitPath` and `recoveryState`. Keep old localStorage entries valid. Existing `.wav/.txt` pairs without a commit remain readable as `legacy`, not retroactively hash-verified.
...
- [ ] Legacy live artifacts remain readable.
