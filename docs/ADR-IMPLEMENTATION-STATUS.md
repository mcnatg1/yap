...
**As of:** 2026-07-13; the stock-NSIS Phase 3 implementation candidate `c3999b7b685dd668165d54b64d1af61e41adad05` passed the one-time local/native/server/GB10 gate, and implementation head `a721121315c7a4bf5510212196141f17e9b237bd` passed hosted CI and the disposable-Windows installer lifecycle.
...
| [0013](adr/0013-global-hotkey-injection.md) | Accepted; Windows implementation active | Client | **180** | Dual safe defaults, explicitly armed physical-code recording, normalization and reserved/conflict rejection, Cancel/per-action Reset, transactional registration rollback, one exact-bounds non-focusable island, target revalidation, Unicode injection, clipboard fallback, focused native WDIO, stock installer contract, and a passing hosted disposable-Windows lifecycle | No macOS/Linux injection adapters, broad real-app/elevation matrix, or verified local real-model/hardware lifecycle on this machine | Expand native compatibility and hardware evidence without weakening the transactional/injection boundary |
