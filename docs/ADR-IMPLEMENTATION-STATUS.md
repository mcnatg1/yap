...
**As of:** 2026-07-13; the stock-NSIS Phase 3 implementation candidate `c3999b7b685dd668165d54b64d1af61e41adad05` passed the one-time local/native/server/GB10 gate. Checked-head hosted CI and the disposable-Windows installer lifecycle remain required before merge.
...
| [0013](adr/0013-global-hotkey-injection.md) | Accepted; Windows implementation active | Client | **160** | Dual shortcut commands, transactional registration, persistence, non-focusable overlay, target revalidation, Unicode injection, clipboard fallback, required native WDIO, stock installer contract, and disposable lifecycle harness | Checked-head hosted installer lifecycle proof is pending; settings still use typed chord fields; paste-last has no default; no deliberate chord recorder/Cancel/Reset flow, macOS/Linux adapters, or broad real-app/elevation matrix | Pass the hosted lifecycle gate, then replace the text fields with safe physical-chord capture and usable defaults and expand native compatibility evidence |
...
| [0019](adr/0019-local-streaming-model-selection.md) | Canonical client fallback decision | Client | **175** | Pinned model revision/SHA artifacts, in-process sherpa Nemotron, warm lifecycle, setup controls, profiler, native tests, and local release/packaging contracts | No checked-head hosted installer lifecycle, completed model-artifact license review, licensed speech/WER fixture, real-model CI accuracy smoke, or cross-platform evidence | Close hosted lifecycle, licensing, fixture, accuracy, and cross-platform gates |
...
The one-time Phase 3 implementation gate ran against exact candidate
`c3999b7b685dd668165d54b64d1af61e41adad05`. The evidence-only documentation
commit that follows does not change executable behavior; hosted checks still
must pass on the final PR head.
...
- Installer closure: Yap targets Tauri's canonical app-data namespace, uses stock NSIS without custom hooks, and has one disposable-Windows lifecycle harness. Hosted lifecycle proof is still required on the checked PR head.
...
The GB10 run did not prove a persistent service or same-process native UI transition, and it introduced no upload, WSS, authentication, ASR, external listener, or firewall change. The local machine did not have `cargo-audit`; the checksum-pinned RustSec audit remains a required hosted-CI lane.
