...
| **4** | Exact-head reference slice verified; reviewed PR pending | Executable candidate `309a2d427707e3483b2649f13940bd48dfaee836` passed the one-time local/native/server/GB10 matrix. Its transient isolated ARM64 worker ran the locked Cohere revision on NVIDIA GB10 in CUDA/BF16 at WER `0.0`; immutable evidence confirmed matching before/after listener, firewall-policy, and service-unit observations plus complete container/worker teardown. No upload/drain, advertised batch capability, authenticated owner derivation, durable server queue, streaming pool, long-recording or multi-worker capacity result, service unit, persistent process, external listener, or firewall change exists. |
